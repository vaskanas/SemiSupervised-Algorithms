...
class SklearnAdapter(Model):
    ...
    def train(self, dataset, *args, **kwargs):
        print('here in partial')
        return self._model.partial_fit(*(dataset.format_sklearn() + args), **kwargs)
...
class SklearnProbaAdapter(ProbabilisticModel):
    ...
    def train(self, dataset, *args, **kwargs):
        ...
        #return self._model.fit(*(dataset.format_sklearn() + args ), **kwargs) # use this line for conventional batch-mode learning
        return self._model.partial_fit(*(dataset.format_sklearn() + args ), classes = np.unique(dataset.format_sklearn()[1]) ) # use this line for incremental learning, for binary datasets
